...
class NetworkHandler:
    ...
    def setup(self):
        ...
        try_run_sudo(
            f"create_ap --freq-band 2.4 --daemon -i {config.INTERNET_IFACE} --ap {config.AP_IFACE} {config.AP_SSID} -p {config.AP_PASSWORD}"
        )

        [
            try_run_sudo(
                f"{cmd} -t nat -A PREROUTING -i wlan0 -p tcp --dport {port} -j REDIRECT --to-port {config.MITMPROXY_LISTEN_PORT}"
            )
