...
class MitmUser(object):
    ...
    def process_pre_key_bundle(
            self, address: ProtocolAddress, pre_key_bundle: state.PreKeyBundle
    ):
    ...
    # def save_bundle(self, address: ProtocolAddress):
    ...
    def decrypt(
            self, address: ProtocolAddress, ciphertext: protocol.CiphertextMessage
    ):
...
class Mitm:
    ...
    def receive_message_from_victim(self, address: ProtocolAddress, ciphertext: protocol.CiphertextMessage):
    ...
    def receive_message_from_user(self, address: ProtocolAddress, ciphertext: protocol.CiphertextMessage):
    ...
    def send_message_to_victim(self, address: ProtocolAddress, plaintext: bytes):
    ...
    def send_message_to_user(self, address: ProtocolAddress, plaintext: bytes):
    ...
    def receive_pre_key_bundle(self, address: ProtocolAddress, pre_key_bundle: state.PreKeyBundle):
        self.fake_victim.process_pre_key_bundle(address, pre_key_bundle)

    def intercept_and_modifying(self, address: ProtocolAddress, ciphertext: protocol.CiphertextMessage):
...
def test_alice_to_bob():
    ...
    Alice = MitmUser(address=ProtocolAddress("alice", 1), RID=1)
    Bob = MitmUser(address=ProtocolAddress("bob", 1), RID=2)
...
def test_mitm_alice_to_bob():
    ...
    Real_Alice = MitmUser(address=ProtocolAddress("alice", 1), RID=1)
    Real_Bob = MitmUser(address=ProtocolAddress("bob", 1), RID=2)

    Fake_Bob = MitmUser(address=ProtocolAddress("bob", 1), RID=2)
    Fake_Alice = MitmUser(address=ProtocolAddress("alice", 1), RID=1)
