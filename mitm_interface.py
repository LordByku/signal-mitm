...
class MitmUser(object):
    def __init__(self, *args, **kwargs):
        ...
        self.identity_key_pair = kwargs.get(
            "identity_key", identity_key.IdentityKeyPair.generate()
        )
        ...
        self.signed_pre_key_id: SignedPreKeyId = SignedPreKeyId(
            kwargs.get("signed_pre_key_id", 22)
        )
        self.signed_pre_key_pair: KeyPair = kwargs.get(
            "signed_pre_key", KeyPair.generate()
        )
        self.signed_pre_key_public: bytes = (
            self.signed_pre_key_pair.public_key().serialize()
        )
        ...
        assert self.identity_key_pair.public_key().verify_signature(
            self.signed_pre_key_public, self.signed_pre_key_signature
        ), "Bad SPK: invalid signature"

        self.store.save_signed_pre_key(
        ...
        self.store.save_pre_key(
            self.pre_key_id, PreKeyRecord(self.pre_key_id, self.pre_key_pair)
        )
        ...
        if "kyber_record" in kwargs:
            ...
            temp_kyber = KyberPreKeyRecord.generate(
                kem_type, self.kyber_pre_key_id, self.identity_key_pair.private_key()
            )

        self.kyber_pre_key_pair = temp_kyber.key_pair()
        self.kyber_pre_key_signature = (
            temp_kyber.signature()
        )  ## if it meeeps, blame chrissy
        ...
        self.last_resort_kyber_pre_key_id = KyberPreKeyId(
            kwargs.get("last_resort_kyber_pre_key_id", 25)
        ...
        if "last_resort_kyber_record" in kwargs:
            ...
            temp_last_resort = KyberPreKeyRecord.generate(
                kem_type,
                self.last_resort_kyber_pre_key_id,
                self.identity_key_pair.private_key(),
            )
        self.last_resort_kyber = temp_last_resort.key_pair()
        self.store.save_kyber_pre_key(
            self.last_resort_kyber_pre_key_id, temp_last_resort
        )  ## todo, make sure the id doesn't match the normal kyber one ^^
        # self.prekey = None
    ...
    def __getstate__(self):
        ...
        state["pre_key_pair"] = (
            self.pre_key_pair.public_key().to_base64(),
            self.pre_key_pair.private_key().to_base64(),
        )
        # del self.store

        state["signed_pre_key_pair"] = (
            self.signed_pre_key_pair.public_key().to_base64(),
            self.signed_pre_key_pair.private_key().to_base64(),
        )
        del self.signed_pre_key_public
        state["signed_pre_key_signature"] = base64.b64encode(
            self.signed_pre_key_signature
        ).decode()
        ...
        state["kyber_pre_key_signature"] = base64.b64encode(
            self.kyber_pre_key_signature
        ).decode()
    ...
    def __setstate__(self, state):
        ...
        self.identity_key_pair = IdentityKeyPair.from_base64(
            state["identity_key_pair"].encode()
        )
        ...
        self.signed_pre_key_signature = base64.b64decode(
        ...
        self.kyber_pre_key_signature = base64.b64decode(
            state["kyber_pre_key_signature"]
        )
        self.last_resort_kyber = kem.KeyPair.from_base64(
            state["last_resort_kyber"][0].encode(),
            state["last_resort_kyber"][1].encode(),
        )
        self.kyber_pre_key_pair = kem.KeyPair.from_base64(
            state["kyber_pre_key_pair"][0].encode(),
            state["kyber_pre_key_pair"][1].encode(),
        )
    ...
    def process_pre_key_bundle(
        self, address: ProtocolAddress, pre_key_bundle: PreKeyBundle
    ):
        session.process_prekey_bundle(address, self.store, pre_key_bundle)

    def encrypt(self, address: ProtocolAddress, plaintext: bytes) -> CiphertextMessage:
        ...
        return session_cipher.message_encrypt(self.store, address, padded_ptxt)

    def decrypt(self, address: ProtocolAddress, ciphertext: bytes) -> Content:
        try:
...
if __name__ == "__main__":
    ...
    bob_signed_pre_key_public = base64.b64decode(
        example_bundle["devices"][0]["signedPreKey"]["publicKey"]
    )
    bob_pre_key_public = base64.b64decode(
        example_bundle["devices"][0]["preKey"]["publicKey"]
    )

    print(
        base64.b64decode(
            example_bundle["devices"][0]["signedPreKey"]["signature"] + "=="
        ).hex()
    )
    ...
    bob_bundle = PreKeyBundle(
        ...
        DeviceId(1),
        (
            PreKeyId(example_bundle["devices"][0]["preKey"]["keyId"]),
            PublicKey.deserialize(bob_pre_key_public),
        ),
        SignedPreKeyId(1),
    ...
    bob_kyber_pre_key_public = base64.b64decode(
        example_bundle["devices"][0]["pqPreKey"]["publicKey"]
    )
    bob_kyber_pre_key_signature = base64.b64decode(
        example_bundle["devices"][0]["pqPreKey"]["signature"] + "=="
    )
    bob_kyber_pre_key_id = example_bundle["devices"][0]["pqPreKey"]["keyId"]

    bob_bundle = bob_bundle.with_kyber_pre_key(
        KyberPreKeyId(bob_kyber_pre_key_id),
    ...
    print(
        (
            bob_bundle.signed_pre_key_public().serialize().hex(),
            bob_bundle.signed_pre_key_signature().hex(),
        )
    )
    ...
    assert (
        enc.message_type() == 3
    )  # PreKey Signal message https://github.com/signalapp/libsignal/blob/f2ae8436b365f5e4e1371102f4702f51ac34e02c/rust/protocol/src/protocol.rs#L33C1-L38
